...
import { ok, serverError, badResquest } from '../../helpers/http-helpers';
...
describe('Signup Controller', () => {
  ...
  test('Should return 400 if no password confirmation is provider', async () => {
    ...
    expect(httpResponse).toEqual(
      badResquest(new MissingParamError('passwordConfirmation')),
    );
  });

  test('Should return 400 if invalid passowrd confirmation provider', async () => {
    const { sut } = makeSut();
    const httpRequest = {
      body: {
        name: 'any_name',
        email: 'any_email@mail.com',
        password: 'any_password',
        passwordConfirmation: 'invalida_password_confirmation',
      },
    };
    const httpResponse = await sut.handle(httpRequest);

    expect(httpResponse).toEqual(
      badResquest(new InvalidParamError('passwordConfirmation')),
    );
  });
